-- =====================================================
-- FINAL TRANSACTION HISTORY SETUP FOR TIC GLOBAL
-- =====================================================
-- This safely creates/updates tables for transaction history
-- Handles existing policies and tables properly
...
-- 4. Create payment_methods table if it doesn't exist
...
DO $$
BEGIN
    -- Check and create user_wallets policies
    ...
    -- Check and create deposits policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'deposits' AND policyname = 'Users can view own deposits') THEN
        CREATE POLICY "Users can view own deposits" ON public.deposits
            FOR SELECT USING (auth.jwt() ->> 'email' = user_email OR auth.uid()::text = user_id::text);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'deposits' AND policyname = 'Users can create own deposits') THEN
        CREATE POLICY "Users can create own deposits" ON public.deposits
            FOR INSERT WITH CHECK (auth.jwt() ->> 'email' = user_email);
    END IF;

    -- Check and create withdrawal_requests policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'withdrawal_requests' AND policyname = 'Users can view own withdrawals') THEN
        CREATE POLICY "Users can view own withdrawals" ON public.withdrawal_requests
            FOR SELECT USING (auth.jwt() ->> 'email' = user_email);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'withdrawal_requests' AND policyname = 'Users can create own withdrawals') THEN
        CREATE POLICY "Users can create own withdrawals" ON public.withdrawal_requests
            FOR INSERT WITH CHECK (auth.jwt() ->> 'email' = user_email);
    END IF;

    -- Check and create wallet_transactions policies
    ...
    -- Check and create payment_methods policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'payment_methods' AND policyname = 'Everyone can view payment methods') THEN
        CREATE POLICY "Everyone can view payment methods" ON public.payment_methods
            FOR SELECT USING (true);
    END IF;
END $$;

-- 9. Insert default payment method (only if it doesn't exist)
INSERT INTO public.payment_methods (method_id, name, network, symbol, wallet_address, min_amount, max_amount, processing_time) 
SELECT 'usdt-trc20', 'USDT', 'TRC20', 'USDT', 'TEHq42c1Kyhe7ShAWYDS61c7aYKNPsT8PF', 10.00, 10000.00, 'Instant - 24 hours'
WHERE NOT EXISTS (SELECT 1 FROM public.payment_methods WHERE method_id = 'usdt-trc20');
...
SELECT 'Final transaction history setup completed successfully!' as result;
