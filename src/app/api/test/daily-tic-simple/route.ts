...
// Simple test endpoint for daily TIC distribution without authentication
export async function POST(request: NextRequest) {
  try {
    console.log('🧪 Starting simple daily TIC distribution test...');
    ...
    for (const subscription of activeSubscriptions) {
      try {
        ...
        if (existingDistribution) {
          console.log(`Tokens already distributed today for ${subscription.user_email} (${subscription.plan_name})`);
          skipped++;
          results.push({
            user_email: subscription.user_email,
            plan_id: subscription.plan_id,
    ...
    console.log(`🎉 Simple daily TIC distribution completed: ${distributed} distributed, ${skipped} skipped, ${errors} errors`);

    return NextResponse.json({
      success: true,
      message: `Simple daily TIC distribution completed for ${today}`,
      date: today,
    ...
    console.error('Error in simple daily TIC distribution:', error);
    return NextResponse.json(
      { 
