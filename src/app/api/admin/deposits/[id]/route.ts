...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (readErr) {
      console.error("DEPOSITS: read error:", readErr);
    ...
    if (upErr) {
      console.error("DEPOSITS: update error:", upErr);
    ...
    console.error("DEPOSITS route uncaught error:", err);
    return NextResponse.json(
      { success: false, message: err?.message || "Unknown server error in /deposits" },
      { status: 500 }
    );
  }
}
