-- QUICK FIX V3: Create payment system tables (skip existing constraints)
-- Run this to fix the "failed to retrieve details" error
...
-- 2. Create payment_transactions table (no foreign keys)
...
-- 3. Create user_plan_subscriptions table
...
-- 4. Skip constraint creation since it already exists
-- The unique_user_plan_subscription constraint already exists, so we skip this step

-- 5. Create indexes (IF NOT EXISTS handles duplicates)
...
-- 6. Insert payment plans (ON CONFLICT handles duplicates)
...
-- 7. Create the payment processing function
...
SELECT plan_id, name, price FROM payment_plans ORDER BY sort_order;
