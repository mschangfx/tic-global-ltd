-- Create dedicated payment_transactions table for plan purchases
-- This keeps payment transactions separate from deposit/withdrawal transactions

-- STEP 0: Create payment_plans table first (required for foreign key)
...
-- STEP 1: Create user_plan_subscriptions table
...
CREATE INDEX IF NOT EXISTS idx_payment_transactions_user_plan ON payment_transactions(user_email, plan_id);
...
-- 4. Create trigger for payment_transactions table
...
-- 5. Create updated payment processing function that uses the new table
...
-- 6. Create function to get user's payment history
CREATE OR REPLACE FUNCTION get_user_payment_history(user_email_param VARCHAR(255))
RETURNS TABLE (
    id UUID,
...
BEGIN
    ...
    WHERE pt.user_email = user_email_param
...
-- 8. Create RLS policy for users to see only their own payment transactions
...
-- 10. Show sample of the new structure
